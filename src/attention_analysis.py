...
class AttentionAnalyzer:
    ...
    def analyze_head_importance(
        ...
        """
        logger.info(f"Analyzing attention head importance using {method} method...")
        
        head_scores = defaultdict(list)
        
        if method == "gradient":
            head_scores = self._gradient_based_importance(dataloader, num_batches)
        elif method == "variance":
            head_scores = self._variance_based_importance(dataloader, num_batches)
        else:
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in batch:
                if torch.is_tensor(batch[key]):
                    batch[key] = batch[key].to(next(self.model.parameters()).device)
    ...
    def _variance_based_importance(self, dataloader, num_batches: int) -> Dict[str, List]:
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                # Extract attention patterns
                patterns = self.extract_attention_patterns(batch, save_patterns=False)
    ...
    def _compute_variance_scores(self, patterns: Dict, head_scores: Dict[str, List]):
        ...
        if patterns['memory_attention'] is not None:
            variance = torch.var(patterns['memory_attention'], dim=-1).mean().item()
    ...
    def analyze_cross_modal_alignment(self) -> Dict[str, float]:
        ...
        for pattern in self.cross_modal_patterns:
            ...
            text_pooled = np.mean(text_features, axis=1)  # [batch_size, feature_dim]
            
            # Compute similarities
            for i in range(len(text_pooled)):
                # Cosine similarity
                cos_sim = np.dot(text_pooled[i], vision_features[i]) / (
                    np.linalg.norm(text_pooled[i]) * np.linalg.norm(vision_features[i])
                )
    ...
    def _plot_cross_modal_attention(
        ...
        for layer_name, attention in cross_attention.items():
            ...
            attn_data = attention[sample_idx].cpu().numpy()  # [num_heads, seq_len, 1]
            
            # Create heatmap
            plt.figure(figsize=(12, 8))
            sns.heatmap(
                attn_data.squeeze(-1).T,  # [1, seq_len] -> [seq_len, num_heads]
                cmap='Blues',
                cbar=True,
            ...
            if save_plots:
                plt.savefig(self.save_dir / f'cross_modal_{layer_name}_sample_{sample_idx}.png')
            plt.close()
    ...
    def _plot_memory_attention(
        ...
        if save_plots:
            plt.savefig(self.save_dir / f'memory_attention_sample_{sample_idx}.png')
        plt.close()
