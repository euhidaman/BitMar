...
class BabyLMMultimodalDataset(Dataset):
    ...
    def _load_captions(self) -> List[str]:
        ...
        try:
            ...
            if isinstance(captions, list):
                ...
                raise ValueError(f"Unexpected captions format: {type(captions)}")
    ...
    def _load_vision_features(self) -> np.ndarray:
        ...
        logger.info(f"Loading vision features from {self.vision_features_1} and {self.vision_features_2}")
...
class BabyLMDataModule:
    ...
    def __init__(self, config: Dict):
        self.config = config
        self.tokenizer_name = config.get('text_encoder_name', 'bert-base-uncased')
