...
class EpisodicMemory(nn.Module):
    ...
    def read_memory(self, query: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Attention scores
        attention_scores = torch.matmul(q, k.transpose(0, 1)) / math.sqrt(self.episode_dim)
        attention_weights = F.softmax(attention_scores, dim=-1)  # [batch_size, memory_size]
        
        # Weighted memory retrieval
        retrieved = torch.matmul(attention_weights, v)  # [batch_size, episode_dim]
...
class BitMarModel(nn.Module):
    ...
    def encode_text(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        ...
        text_features, attention_patterns = self.text_encoder(input_ids=input_ids, attention_mask=attention_mask)
    ...
    def encode_vision(self, vision_features: torch.Tensor) -> torch.Tensor:
        ...
        vision_latent = self.vision_encoder(vision_features)  # [batch_size, vision_latent_size]
    ...
    def create_episode(
        ...
        text_pooled = text_features.mean(dim=1)  # [batch_size, text_encoder_dim]
        
        # Project text to episode dimension if needed
        if text_pooled.size(-1) != vision_latent.size(-1):
            episode_proj = BitNetLinear(text_pooled.size(-1), vision_latent.size(-1)).to(text_pooled.device)
    ...
    def forward(
        ...
        memory_context = self.memory_to_decoder(retrieved_memory)  # [batch_size, fusion_hidden_size]
        ...
        decoder_input = self.decoder_input_proj(fused_with_memory)  # [batch_size, seq_len, text_decoder_dim]
...
def create_bitmar_model(config: Dict) -> BitMarModel:
    ...
    logger.info(f"BitMar Model created with {param_count['total_parameters']:,} total parameters")
