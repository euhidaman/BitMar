...
def test_captions_file():
    ...
    try:
        ...
        # Analyze captions
        if sample_captions:
            avg_length = np.mean([len(caption.split()) for caption in sample_captions])
            max_length = max([len(caption.split()) for caption in sample_captions])
...
def test_vision_features():
    ...
    for i, file_path in enumerate(vision_files):
        ...
        try:
            ...
            logger.info(f"   Range: [{features.min():.3f}, {features.max():.3f}]")
    ...
    try:
        ...
        logger.info(f"   Memory usage: {combined_features.nbytes / (1024**2):.1f} MB")
...
def test_data_module():
    ...
    try:
        ...
        logger.info(f"   Train batch input shape: {train_batch['input_ids'].shape}")
        logger.info(f"   Train batch vision shape: {train_batch['vision_features'].shape}")
        logger.info(f"   Val batch input shape: {val_batch['input_ids'].shape}")
        logger.info(f"   Val batch vision shape: {val_batch['vision_features'].shape}")
...
def test_data_consistency():
    ...
    try:
        ...
        if min_count == max_count:
            ...
            logger.warning(f"⚠️  Count mismatch: will use {min_count:,} samples")
...
def test_tokenization():
    ...
    try:
        ...
        for i, caption in enumerate(test_captions):
            ...
            logger.info(f"   Caption {i+1}: {len(caption.split())} words -> {attention_mask.sum().item()} tokens")
...
def run_dataset_compatibility_test():
    ...
    try:
        ...
        if all(file_results.values()):
            ...
            logger.info("Please ensure BabyLM dataset is downloaded to ../babylm_dataset/")
            return False
