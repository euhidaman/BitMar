...
class BitMarEvaluator:
    ...
    def evaluate_cross_modal_retrieval(self, num_samples: int = 100) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(val_loader)):
                ...
                text_latent = outputs['text_latent']
        ...
        # Compute metrics
        retrieval_accuracy = correct_retrievals / \
    ...
    def evaluate_memory_usage(self, num_samples: int = 100) -> Dict[str, float]:
        ...
        if memory_patterns:
            ...
            logger.info(
                f"  Usage uniformity: {results['usage_uniformity']:.4f}")
    ...
    def evaluate_generation_quality(self, num_samples: int = 50) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader):
                ...
                for i in range(batch_size):
                    ...
                    input_ids_partial[0,
                                      half_length:] = self.tokenizer.pad_token_id
        ...
        avg_generated_length = np.mean(
            [len(text.split()) for text in generated_texts])
        avg_reference_length = np.mean(
        ...
        logger.info(
            f"  Avg generated length: {avg_generated_length:.2f} words")
        logger.info(
            f"  Avg reference length: {avg_reference_length:.2f} words")
...
def main():
    ...
    if 'cross_modal_retrieval' in results:
        ...
        logger.info(
            f"Cross-Modal Retrieval Accuracy: {retrieval.get('retrieval_accuracy', 0):.4f}")

    if 'memory_usage' in results:
        memory = results['memory_usage']
        logger.info(
            f"Memory Usage Entropy: {memory.get('average_entropy', 0):.4f}")
        logger.info(
            f"Memory Usage Uniformity: {memory.get('usage_uniformity', 0):.4f}")

    if 'generation_quality' in results:
        generation = results['generation_quality']
        logger.info(
            f"Generation Vocab Overlap: {generation.get('average_vocab_overlap', 0):.4f}")
