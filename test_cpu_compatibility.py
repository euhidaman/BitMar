...
def test_data_loading():
    ...
    try:
        ...
        logger.info(
            f"   Sample vision shape: {sample['vision_features'].shape}")
...
def test_model_inference(model, data_module):
    ...
    try:
        ...
        logger.info(
            f"   Vision latent shape: {outputs['vision_latent'].shape}")

        if outputs['memory_usage'] is not None:
            logger.info(
                f"   Memory usage pattern: {outputs['memory_usage'][:10].tolist()}")
...
def test_quantization():
    ...
    try:
        ...
        threshold = 0.5
        quantized = torch.sign(weight_normalized) * \
            (weight_normalized.abs() > threshold)
        ...
        logger.info(
            f"   Original weight range: [{test_weight.min():.3f}, {test_weight.max():.3f}]")
...
def run_cpu_compatibility_test():
    ...
    if tests_passed == total_tests:
        ...
        logger.info("\n📋 Next steps:")
        logger.info(
            "1. Run dataset compatibility test: python test_dataset_compatibility.py")
        logger.info("2. For GPU training, push to GitHub and run on RunPod")
        logger.info("3. Use full configuration for GPU training")
    else:
        logger.warning(
            f"⚠️  {total_tests - tests_passed} tests failed. Check logs above.")
