...
class BitMarTrainer:
    ...
    def setup_optimizer(self):
        ...
        logger.info(f"Optimizer: AdamW with LR={self.config['model']['learning_rate']}")
        if self.scheduler:
    ...
    def run_attention_analysis(self):
        ...
        analysis_data_module.setup(max_samples=1000)  # Limit samples for analysis
    ...
    def train(self, max_samples: Optional[int] = None):
        ...
        for epoch in range(self.current_epoch, self.config['training']['max_epochs']):
            ...
            logger.info(f"Memory Entropy: {train_metrics['memory_usage_entropy']:.4f}")
            logger.info(f"Cross-Modal Similarity: {train_metrics['cross_modal_similarity']:.4f}")
